...
namespace Neuron
{
	...
	{		
		...
		{
			// create material
			string path = "Assets/Noitom/PerceptionNeuron/Resources/Shaders/BoneLineShader.shader";
			Shader shader = UnityEditor.AssetDatabase.LoadAssetAtPath( path, typeof( Shader ) ) as Shader;
            if(shader == null)
                shader = UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/Noitom/PerceptionNeuron/Resources/Shaders/BoneLineShader.shader", typeof(Shader)) as Shader;
            if ( shader != null )
			{
