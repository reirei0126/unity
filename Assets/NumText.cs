    ...
    IEnumerator cut()
    {
        ...
        while (Time.time < 64.0f)
        {
            ...
            for (int j = 0; j < Bones.Length; j++)
            {
                ...
                ave += maxstop;
                boneStop = (double)maxstop * 0.011;
                Texts[j].text = string.Format("{0}:{1:f4}s", BoneNames[j],boneStop);
                /*
                if (maxstop > 13)
                {
            ...
            boneAveStop = (double)ave * 0.011;
            Judge.text = string.Format("ストップ時間：{0:f4}s",boneAveStop);
            /*
           if (ave > 13)
           {
